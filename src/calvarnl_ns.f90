...
c viscosity: no-slip boundary condition
c wind forcing
...
c
     ...
     &            ak(nnmod),b(nnmod),bu(nnmod),bv(nnmod),
...
c$&                  nc,nm,normt,itm),
c$&            local(i,j,i1,i2,i3,ax,bx,ay,by,deta,detai,
...
c
c$omp parallel share(u0,v0,h0,w0,hb,qxx,qxy,qyx,qyy,w1,z1,uh,vh,fc,
c$&                  nc,nm,normt,itm,mb0),
c$&            local(i,j,
...
c
c------------------------------------------------
c projection onto the zero-at-the-boundary basis functions
c
	   call reduc_gn(nc,uhk,bu,cip)
	   call reduc_gn(nc,vhk,bv,cip)
c
c------------------------------------------------
c
	   call dtrsm4(nm-nc-1,amml,bu,dia2)
	   call dtrsm4(nm-nc-1,amml,bv,dia2)
c
c------------------------------------------------
c projection back
c
	   call reduc_inv_gn(nc,uhk,bu,cip)
	   call reduc_inv_gn(nc,vhk,bv,cip)
c------------------------------------------------
...
c	  dny = normt(2,i)
c
c	   call rot_vec(nm,gn,gt,uhk,vhk,dnx,dny)
c	   call reduc_gn(nc,gn,bu,cip)
c
c	   call dtrsm4(nm-nc-1,amml,bu,dia2)
c	   call dtrsm4(nm,     amm, gt,dia1)
c
c	   call reduc_inv_gn(nc,gn,bu,cip)
c	   call rot_vec(nm,uhk,vhk,gn,gt,dnx,dny)
c
	else
c
...
c
c------------------------------------------------
c velocity increment set to zero at the boundary
c------------------------------------------------
c
	 if (bdl(i)) then
c
...
c projection onto the zero-at-the-boundary basis functions
c
	   call reduc_gn(nc,gx,bu,cip)
	   call reduc_gn(nc,gy,bv,cip)
c
c------------------------------------------------
c
	   call dtrsm4(nm-nc-1,amml,bu,dia2)
...
c projection back
c
	   call reduc_inv_gn(nc,gx,bu,cip)
	   call reduc_inv_gn(nc,gy,bv,cip)
c
c
c-------------------------------------------------
